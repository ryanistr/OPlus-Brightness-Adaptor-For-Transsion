...
            s     // state is explicitly off or doze/doze_suspend
...
                    if dbg { ld("[DisplayAdaptor] Panoramic AOD enabled, waiting for screen off"); }
                    last_val // don't set to 0
                } else {
                    if dbg { ld("[DisplayAdaptor] Panoramic AOD disabled, setting brightness off"); }
                    F_Z // set to 0
                }
            } else {
...
s               wb(fd, val_to_write, &mut last_val, dbg);
            }
        }
